...
- Jangan pakai localStorage/sessionStorage untuk data inti — pakai DB.
...
- `User` (id, nama, role, areaId?) — role: ADMIN, KOMITE, AUDITOR, AUDITEE, REDTAG, MANAGEMENT
...
> Catatan implementasi: SQLite via Prisma tidak mendukung enum & scalar list, jadi enum disimpan sebagai `String` (nilai sah didokumentasikan di schema) dan daftar foto sebagai JSON string `fotoPaths`.
---
## 9. Roadmap Build (urutan modul)
Demo-ready setelah **Modul 0–4**. Bangun bertahap, jangan loncat.
| # | Modul | Isi | Demo-ready? |
|---|---|---|---|
| **0** | Setup | Next.js 14 + Prisma + SQLite + Tailwind, schema awal, seed data (12 area, user tiap role, guiding questions, contoh temuan) | ✅ selesai |
...
- **Modul 0 SELESAI**: Next.js 14 + TS + Tailwind + Prisma + SQLite. Schema lengkap (§8), seed dummy realistis (12 area, 11 user 6 role, 20 guiding question, checklist 3 area, 6 red tag, siklus baseline 2026-04 + siklus aktif 2026-06). Fungsi scoring murni `src/lib/scoring.ts` + validasi baseline `scripts/check-scoring.ts` (95.0/99.0/100.0 lulus).
- Prototype Vite lama diarsipkan di `/legacy` sebagai referensi UI saja.
- **Berikutnya: Modul 1** (Auth & Role Shell) — menunggu konfirmasi.
